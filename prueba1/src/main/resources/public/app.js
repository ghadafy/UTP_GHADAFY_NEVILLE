...
async function enviarDatos(miarreglo, matrizSol) {
    const datos = {
    ...
    await fetch("./piramide", {
        ...
        .then(data => console.log("PiramideEnviada: ", data))
    ...
    await fetch(url)
        ...
        .then(data => {
            ...
            data.forEach(info => {
                ...
                let tamano = info.fila.datos.tamano
                let sol = info.fila.datos.solucion
                if (tamano == 2) {
                    imagen.src = 'imgs/nivel2.png'
                }
                else if (tamano == 3) {
                    imagen.src = 'imgs/nivel3.png'
                }
                else if (tamano == 4) {
                    imagen.src = 'imgs/nivel4.png'
                }
                else if (tamano == 5) {
                    imagen.src = 'imgs/nivel5.png'
                }
                else {
                    imagen.src = 'imgs/mayor5.png'
                }
                div.classList.add("filaNueva")
                div.setAttribute('id', info.id)
                div.appendChild(p)
                p.innerHTML = `(${info.id + 1}) - N ${info.fila.datos.tamano} - P ${sol[0][0]} `

                ventanaNueva(info.id, div); //Aqui escuchamos el evento que abre las ventanas
                // document.querySelector('#historial').appendChild(div).appendChild(imagen)
